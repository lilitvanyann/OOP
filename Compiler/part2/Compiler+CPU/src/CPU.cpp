...
void CPUSimulator::execute(uint32_t instr) {
    ...
    case OP_LI:  cpu.R[rd] = instr & 0xFFFF; break;  // 16-bit immediate

    case OP_LOAD: cpu.R[rd] = loadWord(cpu.R[rs1]); break;
    case OP_STORE: storeWord(cpu.R[rs1], cpu.R[rd]); break; 

    case OP_JMP:  cpu.PC = instr & 0xFFFFFF; jump = true; break;
    case OP_CALL: cpu.RA = cpu.PC + 4; cpu.PC = instr & 0xFFFFFF; jump = true; break;
    ...
    case OP_JLT: if (cpu.negative_flag) { cpu.PC = instr & 0xFFFFFF; jump = true; } break;
    case OP_JGT: if (!cpu.negative_flag && !cpu.zero_flag) { cpu.PC = instr & 0xFFFFFF; jump = true; } break;
    case OP_JLE: if (cpu.negative_flag || cpu.zero_flag) { cpu.PC = instr & 0xFFFFFF; jump = true; } break;
    case OP_JGE: if (!cpu.negative_flag) { cpu.PC = instr & 0xFFFFFF; jump = true; } break;
    case OP_JEQ: if (cpu.zero_flag) { cpu.PC = instr & 0xFFFFFF; jump = true; } break;
    case OP_JNE: if (!cpu.zero_flag) { cpu.PC = instr & 0xFFFFFF; jump = true; } break;
    ...
    default: throw std::runtime_error("Unknown opcode " + std::to_string(opcode) +
        " at PC=" + std::to_string(cpu.PC - 4) +
        " instr=0x" + [](uint32_t v) { char b[9]; snprintf(b, 9, "%08X", v); return std::string(b); }(instr));
    }
